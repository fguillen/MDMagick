...
function MDM( inputElement ) {
  ...
  this.activateControls = function( controlsElement ){
    var _self = this;
    ["bold", "italic", "link", "title", "list", "image"].forEach( function( actionName ){
...
MDM.Actions = {
  ...
  image: function( inputElement ){
    ...
    if( link !== null ) {
      var selection = $( inputElement ).getSelection();
      var result = "![" + selection.text + "](" + link + ")";
      if ( selection.length > 0 ) {
        $( inputElement ).replaceSelection( result );
      } else {
        $( inputElement ).insertAtCaretPos( result );
      }
    }
  }
}
...
MDM.Utils = {
  ...
  controlsTemplate: function(){
    var template =
      ...
      "    <li class=\"mdm-image\"><a class=\"mdm-icon-image\" href=\"#mdm-image\"><span>img</span></a></li>" +
      "  </ul>" +
      "</div>";
